...
async function refreshStats() {
  ...
  if (goPro) {
    ...
    // Var pengarna tar vägen ska stå där de begärs, inte i villkorstexten.
    const payNote = document.getElementById("payNote");
    if (payNote) {
      payNote.innerHTML = pro
        // Den enda frågan någon har innan de klickar "avsluta": förlorar jag tillgången
        // nu? Svaret är nej — Stripe är satt till at_period_end och vår
        // upsert_subscription behåller planen så länge status är aktiv. Det ska stå
        // här, inte upptäckas efteråt.
        ? "Billing is handled by Stripe. You can cancel any time — Pro stays active "
          + "until the end of the month you have already paid for, and nothing is "
          + "charged after that."
        : "Payment is handled by <b>Stripe</b> — card details go straight to them and "
          + "never touch our servers. Cancel any time, from your account.";
    }
...
function wireBilling() {
  ...
  function wireApp() {
    ...
    function jobOptions() {
      ...
      // Auto-chippet bär data-k="" och betyder "ingen preferens" — filtreras bort så
      // att servern får en tom lista, precis som förr när inget var valt.
      const kinds = [...document.querySelectorAll("#kinds .chip.on")]
        .map((c) => c.dataset.k).filter(Boolean);
    ...
    (function wireControls() {
      ...
      if (chips) {
        const auto = chips.querySelector(".chip.auto");
        const press = (el, on) => {
          el.classList.toggle("on", on);
          el.setAttribute("aria-pressed", on ? "true" : "false");
        };
        const sync = () => {
          const picked = [...chips.querySelectorAll(".chip.on")].filter((x) => x.dataset.k);
          // "Automatiskt" och ett konkret val är samma sak sagt två gånger — de kan
          // aldrig lysa samtidigt, och något måste alltid lysa. Ett formulär där
          // ingenting är valt läser som "du har glömt något".
          if (auto) press(auto, picked.length === 0);
          kindsHint.innerHTML = picked.length
            ? `Pushing <b class="gld">${picked.map((x) => x.querySelector("b").textContent).join(" · ")}</b> `
              + "to the front. Other moments still qualify when they are strong enough."
            : "We look for every kind of moment and keep the strongest. Pick one or more to "
              + "push those to the front — they are favoured, never the only ones, so a quiet "
              + "stream still returns its best.";
        };
        chips.addEventListener("click", (e) => {
          const c = e.target.closest(".chip");
          if (!c) return;
          if (c === auto) {
            chips.querySelectorAll(".chip").forEach((x) => press(x, x === auto));
          } else {
            press(c, !c.classList.contains("on"));
          }
          sync();
        });
        sync();
      }
      // Poängrutan: en (i) som faktiskt öppnar något.
      const ptsBtn = document.getElementById("ptsBtn"), ptsBox = document.getElementById("ptsBox");
      if (ptsBtn && ptsBox) {
        ptsBtn.addEventListener("click", () => {
          const open = ptsBox.hasAttribute("hidden");
          if (open) ptsBox.removeAttribute("hidden"); else ptsBox.setAttribute("hidden", "");
          ptsBtn.setAttribute("aria-expanded", open ? "true" : "false");
        });
      }
